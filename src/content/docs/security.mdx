...
1. Download the Granted release artifact you wish to verify (we will use the Linux `x86_64` version as an example):
   ...
   ```
   curl -OL releases.granted.dev/granted/v0.8.0/granted_0.8.0_linux_x86_64.tar.gz
   ```

2. Download the checksums for the release:
   ```bash
   curl -OL releases.granted.dev/granted/v0.8.0/checksums.txt
   ```
3. Download the signature file:

   ```bash
   curl -OL releases.granted.dev/granted/v0.8.0/checksums.txt.sig

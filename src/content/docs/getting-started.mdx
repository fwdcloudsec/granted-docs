...
<Tabs>
  <TabItem label="MacOS">
    ...
    There is also an unofficial `granted` formula in [homebrew-core](https://formulae.brew.sh/formula/granted), which is what a plain `brew install granted` (without the tap) installs. That formula is maintained by the Homebrew maintainers rather than the Granted team and may lag behind our releases, so we recommend installing from the `fwdcloudsec/granted` tap as shown above.
